// Minimal masthead — just the writer's name, with mixed italic styling

const { useState: useS_mh, useEffect: useE_mh } = React;

function Masthead({ data, onAdminPing }) {
  const m = data.meta;
  const [clicks, setClicks] = useS_mh(0);
  function handleTitleClick() {
    const next = clicks + 1;
    setClicks(next);
    if (next >= 3) {setClicks(0);onAdminPing && onAdminPing();}
  }

  // Split name like "Avery J. Ramos" → italic the middle token
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
          ...
          <h1
            ...
            style={{
              ...
              margin: 0,
              cursor: "default",
              userSelect: "none", fontSize: "70px", fontFamily: "sans-serif"
            }}>
            {renderName()}</h1>
        </div>
